...
select
    ...
    s.student_name,
    sub.subject_name,
    COALESCE(count(e.student_id)) as attended_exams
from
    Students s
...
### Notes
- Use ``CROSS JOIN`` to generate all possible combinations of columns.
- Use a ``COALESCE`` function to return 0 if there is no matching row.